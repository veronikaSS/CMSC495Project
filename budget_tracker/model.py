...
class Transaction(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    account = db.Column(db.ForeignKey('account.id'), nullable=True)
    account_owner = db.Column(db.String(200), nullable=True)
    amount = db.Column(db.Float(12), decimal_places=2, nullable=True)
    authorized_date = db.Column(db.DateTime, nullable=True)
    category_id = db.Column(db.String(200), nullable=True)
    date =db.Column(db.DateTime, nullable=False, default=datetime.now())
    iso_currency_code = db.Column(db.String(200), nullable=True)
    location =db.Column(db.String(200), nullable=True)
    merchant_name = db.Column(db.String(200), nullable=True)
    name = db.Column(db.String(200), nullable=True)
    payment_meta = db.Column(db.String(200), nullable=True)
    payment_channel = db.Column(db.String(200), nullable=True)
    pending = db.Column(db.Boolean, nullable=True)
    pending_transaction_id = db.Column(db.String(200), nullable=True)
